    ...
    function OpenCwdButton(props) {
      ...
      function onClick() {
        ...
        if (remote === undefined || remote === null) {
          setFeedback({ kind: "error", text: "remote 服务不可用" });
          return;
        }
        if (remote.session === undefined || remote.session === null) {
          setFeedback({ kind: "error", text: "remote.session 不可用" });
          return;
        }
        ...
        remote.session.openWorkspacePath({ path: path }).then(function (res) {
          if (res !== null && res !== undefined && res.ok === true) {
            setFeedback({ kind: "ok", text: "已请求打开" });
          } else {
            var msg = res !== null && res !== undefined && res.error !== undefined
              ? (res.error.message !== undefined ? res.error.message : String(res.error))
              : "unknown error";
            setFeedback({ kind: "error", text: "Host 返回: " + msg });
          }
        }).catch(function (err) {
          setFeedback({ kind: "error", text: "调用失败: " + (err !== null && err !== undefined && err.message !== undefined ? err.message : String(err)) });
        });
      }
